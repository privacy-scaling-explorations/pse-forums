...
var generateRsaKeys = function() {
  return crypto.subtle.generateKey(rsaAlgorithm, /* extractable: */ true, /* keyUsages: */ ["wrapKey", "unwrapKey"])
    .catch(function(error) {
      throw "Error generating keys."
    })
    .then(function(rsaKey) {
      var exportPublicKey = crypto.subtle.exportKey("spki", rsaKey.publicKey)
        .catch(function(error) {
          throw "Error exporting public key."
        })
      var exportPrivateKey = crypto.subtle.exportKey("pkcs8", rsaKey.privateKey)
        .catch(function(error) {
          throw "Error exporting private key."
        })

      return Promise.all([exportPublicKey, exportPrivateKey])
        .then(function(keys) {
          return { publicKeyBuffer: keys[0], privateKeyBuffer: keys[1] }
        })
    })
}

var rsaEncrypt = function(data, rsaPublicKeyBuffer) {
  var importRsaPublicKey = crypto.subtle.importKey(
    "spki",
    rsaPublicKeyBuffer,
    rsaAlgorithm,
    /* extractable: */ false,
    ...
    .catch(function(error) {
      throw "Error importing public key."
    })
  var generateAesKey = crypto.subtle.generateKey(aesAlgorithm, /* extractable: */ true, /* keyUsages: */ ["encrypt"])
    .catch(function(error) {
      throw "Error generating symmetric key."
    })
...
var rsaDecrypt = function(data, rsaPrivateKeyBuffer) {
  return crypto.subtle.importKey(
    ...
    .catch(function(error) {
      throw "Error importing private key."
    })
    .then(function(rsaKey) {
      ...
      return crypto.subtle.unwrapKey(
        ...
        rsaKey,
        rsaAlgorithm,
        initializedaesAlgorithm,
        /* extractable: */ false,
        /* keyUsages: */ ["decrypt"],
      )
        .catch(function(error) {
          throw "Error decrypting symmetric key."
        })
